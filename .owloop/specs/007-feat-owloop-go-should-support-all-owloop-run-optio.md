...
- Extend the `go` command so it accepts the same runtime options as `run`: `--agent`, `--model`, `--verifier-model`, `--subagents`, `--idle-timeout`, `--max-duration`, `--max-tokens`, and `--worktree/--no-worktree`.
- Forward every parsed option from `go` into the engine runner, replacing the currently hard-coded defaults (`agent="claude"`, `worktree=True`, `idle_timeout=3600`, `max_duration=0`, `max_tokens=0`, no verifier, no subagents).
- Keep `go` unlimited by design; do not expose the `--max-iterations` option that `run` uses.
- Preserve current defaults when options are omitted: agent=claude, worktree enabled, idle_timeout=3600s, max_duration=0, max_tokens=0, verifier_model=None, subagents=False.
- Add unit tests that verify the help text exposes the new options and that parsed option values are forwarded correctly to the engine runner.

## Acceptance Criteria

- [ ] `uv run owloop go --help | grep -q -- --agent && echo ok` → `ok`
- [ ] `uv run owloop go --help | grep -q -- --max-tokens && echo ok` → `ok`
- [ ] `uv run owloop go --help | grep -q -- --max-duration && echo ok` → `ok`
- [ ] `uv run owloop go --help | grep -q -- --subagents && echo ok` → `ok`
- [ ] `uv run owloop go --help | grep -q -- --verifier-model && echo ok` → `ok`
- [ ] `uv run owloop go --help | grep -q -- --worktree && echo ok` → `ok`
